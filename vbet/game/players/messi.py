    ...
    async def forecast(self, competition_id, active_week: int):
        ...
        if not game_data:
            for event_id, event_data in week_games.items():
                ...
                q = competition.table.ready_table.get(target)
                r = competition.table.ready_table.get(opponent)
                team_streak = q.get('streak')  # type: Dict
                k = [x[1] for x in sorted(team_streak.items(), key=operator.itemgetter(0))]
                e = k[active_week - 6: active_week - 1]
                if len(e) == 5:
                    pass
                else:
                    return []
                opponent_streak = r.get('streak')  # type: Dict
                k = [x[1] for x in sorted(opponent_streak.items(), key=operator.itemgetter(0))]
                f = k[active_week - 6: active_week - 1]
                if len(f) == 5:
                    pass
                else:
                    return []
                ...
                if abs(y) >= 11:
                    ...
                    if u and aa:
                        if u == home_away and aa == home_away:
                            if target_r >= secrets.choice([80, 60]) and opponent_r <= 40:
                                if sum(f[3:]) <= 0:
                                    if y > 0 and opponent == player_b:
                                        o_id = 0
                                    else:
                                        o_id = 1
                                    ...
                                    print(home_away, target, opponent, target_odd, opponent_odd, u, v, aa, bb, target_r,
                                          opponent_r, e, f, q.get('pos'), r.get('pos'), o_id)
