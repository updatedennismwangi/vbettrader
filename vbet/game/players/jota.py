...
class CustomPlayer(Player):
    ...
    async def forecast(self, competition_id, active_week: int):
        ...
        if not game_data:
            for top_player in competition.table.table[0:2]:
                ...
                if len(u_streak) >= 5:
                    if sum(u_streak) >= 15:
                        ...
                        if len(v_streak) >= 9:
                            ...
                            game_data['loss_draw'] = 0
                            game_data['start_week'] = active_week
                            break
        if not game_data:
            return []
        loss_draw = game_data.get('loss_draw')
        if loss_draw < 1:
            return []
        target_games = {}
    ...
    async def on_result(self, competition_id: int):
        ...
        if game_data:
            ...
            if len(y) >= self.min_loss_draw:
                h = Counter(y)
                if h.get(0, 0) < 1 < h.get(3, 0):
                    # Activate placing ticket
                    if game_data['loss_draw'] < 1:
                        game_data['loss_draw'] = 1
                    logger.info('%r Check League : %d Team: %s %s %s', competition, competition.league, team, str(y),
                                str(v_streak))
                else:
                    # Code check already broken
                    if game_data.get('loss_draw') < 1:
                        game_data.clear()
