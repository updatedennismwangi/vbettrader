...
class CustomPlayer(Player):
    ...
    def __init__(self, **kwargs):
        ...
        self.target_player = "BAR"
        self.min_over_under = 0
    ...
    async def forecast(self, competition_id, active_week: int):
        ...
        if not game_data:
            t_map = []
            for w in range(active_week - 2, active_week):
                t_data = competition.table.get_week_results(w)
                for event_id, event_result in t_data.items():
                    t_a = event_result.get("A")
                    t_b = event_result.get("B")
                    score = event_result.get('score')
                    if t_a == self.target_player or t_b == self.target_player:
                        if score[0] + score[1] >= 3:
                            t_map.append(0)
                        else:
                            t_map.append(1)
            c = Counter(t_map)
            if c.get(0, 0) >= self.min_over_under:
                logger.info('%r Attempting match League : %d Week: %d', competition, competition.league,
                            active_week)
                game_data['team'] = self.target_player
    ...
    def get_required_weeks(self, competition_id: int):
        ...
        y = [x for x in range(3, competition.max_week + 1)]
