...
class CustomPlayer(Player):
    ...
    async def forecast(self, competition_id, active_week: int):
        ...
        if not game_data:
            ...
            team_a_data, team_b_data = table[len(table) - 2:]
            t_a_p, t_b_p = team_a_data.get('points'), team_b_data.get('points')
            print(t_a_p, t_b_p)
            if t_a_p - t_b_p > 8:
                if active_week <= (competition.max_week - 5):
                    game_data['team'] = team_b_data.get('team')
        if not game_data:
            return []
